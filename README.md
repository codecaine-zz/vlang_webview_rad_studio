...
| ------------------------ | --------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API Studio Pro**       | <img src="resources/screenshots/app_api_studio.png" width="300" alt="API Studio Pro" />             | **Full-Featured REST Client & HTTP Inspector**: Execute GET, POST, PUT, DELETE, PATCH, HEAD with custom headers editor, body editor, preset endpoints, response stopwatch latency, colored status codes, cURL export, and request history table.<br>`v run applications/api_studio.v` |
| **System Studio Pro**    | <img src="resources/screenshots/app_system_studio.png" width="300" alt="System Studio Pro" />       | **Hardware Intelligence & Telemetry Workstation**: Real-time multi-metric KPI dashboard, CPU cores/load averages, RAM utilization, storage partitions, battery sensor, host uptime, and system specifications export.<br>`v run applications/system_studio.v`     |
| **Database Studio Pro**  | <img src="resources/screenshots/app_database_studio.png" width="300" alt="Database Studio Pro" />   | **SQLite Query & Schema Workbench**: Connect to SQLite files or in-memory databases (`:memory:`), discover tables and schema info, execute arbitrary queries with latency stopwatch, browse dynamic result sets, and export results to CSV.<br>`v run applications/database_studio.v` |
| **Git Workbench Studio** | <img src="resources/screenshots/app_git_studio.png" width="300" alt="Git Workbench Studio" />       | **Visual Git Version Control Workbench**: Real-time working-tree status, stage all (`git add -A`), unstage (`git reset`), commit with custom messages, git stash, pull, push, unified diff viewer with monospace styling, and interactive commit log history.<br>`v run applications/git_studio.v` |
| **DevTools Studio Pro**  | <img src="resources/screenshots/app_devtools_studio.png" width="300" alt="DevTools Studio Pro" />   | **Developer Omnitool Suite**: Live casing transforms (camelCase, snake_case, kebab-case, Title Case), slugify, JWT token header/payload decoder, Unix timestamp converter with RFC3339 formatting, and string codecs.<br>`v run applications/devtools_studio.v` |
| **Crypto Studio Pro**    | <img src="resources/screenshots/app_crypto_studio.png" width="300" alt="Crypto Studio Pro" />       | **Cryptography & Security Suite**: Live SHA-256, SHA-512, MD5, SHA-1, HMAC, Base64, and Hex calculation, Shannon entropy analyzer, UUID v4 generator, high-entropy password generator, and file checksum verifier.<br>`v run applications/crypto_studio.v` |
| **Network Studio Pro**   | <img src="resources/screenshots/app_network_studio.png" width="300" alt="Network Studio Pro" />     | **Network Diagnostics & Port Scanner**: Live ICMP ping probe (3 packets), DNS resolution (`nslookup`), common port scanner (80, 443, 22, 8080), HTTP health checks, real-time streaming terminal console, and diagnostic history.<br>`v run applications/network_studio.v`     |
...
| **App Bundler Studio**   | <img src="resources/screenshots/app_app_bundler_studio.png" width="300" alt="App Bundler Studio" /> | **Desktop Application Packager**: Multi-target compiler (`-prod`, `-g`), complete macOS `.app` bundle generator with `Info.plist` generation, and live compiler build console.<br>`v run applications/app_bundler_studio.v` |
| **Environment Studio**   | <img src="resources/screenshots/app_env_studio.png" width="300" alt="Environment Studio" />         | **Environment Variables & Context Workbench**: Complete alphabetical environment variable table, search filter, runtime variable setter, `.env` file export/import, and a system `PATH` directory integrity validator.<br>`v run applications/env_studio.v` |
| **Watcher Studio Pro**   | <img src="resources/screenshots/app_watcher_studio.png" width="300" alt="Watcher Studio Pro" />     | **Enterprise File System Monitor & Automation Pipeline**: Pre-indexed baseline (no false startup events), multi-metric change tracking (`mtime`, `ctime`, `size`), VCS noise exclusion, debounce intervals, dynamic placeholders (`{file}`, `{path}`, `{filename}`, `{event}`, `{dir}`, `{time}`), live execution console, and CSV audit export.<br>`v run applications/watcher_studio.v` |
